...
class LocalGoalsRepository @Inject constructor(private val goalDao: GoalDao) : GoalsRepository {
    ...
    override fun getAllGoals(): Flow<List<Goal>> =
        goalDao.getAllGoals().map { goalEntities ->
            goalEntities.map(PopulatedGoal::asGoal)
        }

    override fun getGoalById(goalId: String): Flow<Goal> =
        goalDao.getGoalById(goalId).map { goalEntity ->
            goalEntity.asGoal()
        }

    override fun getGoalsFromHabit(habitId: String): Flow<List<Goal>> =
        goalDao.getGoalsFromHabit(habitId).map { habitEntities ->
